...
/**
  * Included additional class to store handleMessage information
...
public class MessageManagerService implements MessageManager, Initializable, Activatable
{
    ...
    public void initialize (BrokerContext context)
    {
        ...
        // System.out.println("Connected to Database " + dbname + " from Initialize in MessageManagerService"); 
    }
   ...
   public synchronized void handleMessage (CustomerBootstrapData cbd) {
    ...
    // This method can be used to store relevant information in mongodb database
    // and use stored information for training your model or learning patterns
    // Just uncomment specific code snippets to store relevant information 
    public synchronized void handleMessage(TimeslotComplete ts)
    {
      ...
      // Stores the calendar information about the game
      /* try
      {
      ...
      // Stores the calendar information of future timeslots in the game
      /* try
      {
      ...
      // Stores the weather information of each timeslot in the game
      /* try
      {
      ...
      // Stores the weather forecast information in the game
      /* try
      {
      ...
      // Stores the market clearing information (MCP, MCQ) of each {bidding timeslot, executation timeslot} pair in the game
      /* try
      {
        ...
        for(int proximity = 1; proximity <= 24; proximity++)
        {
          ...
          document5.put("MCP", MCP);                                    // MCP - Market Clearing Price
          document5.put("Net_Cleared_Quantity", netClearedQuantiy);     // MCP - Market Clearing Quantity
      ...
      // Stores the broker's clearing information (MCP, CQ) each {bidding timeslot, executation timeslot} pair in the game (only when bid gets cleared)
      /* try
      {
        ...
        for(Map.Entry<Integer, Pair<Double, Double>> message : MTI.entrySet())
        {
          ...
          document6.put("MCP", message.getValue().getKey());                            // MCP - Market Clearing Price
          document6.put("Broker's_Cleared_Quantity", message.getValue().getValue());    // CQ - Broker's Clearing Quantity
      ...
      // Stores the annonymized uncleared bids and uncleared asks for each timeslot in the game
      /* try
      {
      ...
      // Stores the balancing information for each timeslot in the game
      /* try
      {
      ...
      // Stores the distribution relared information for each timeslot in the game
      /* try
      {
      ...
      // Stores the aggregated clearing information per timeslot in the game
      /* try
      {
      ...
      // Stores the cash and quantity balance information for each timeslot in the game
      /* try
      {
      ...
      // Stores the capacity transaction related information for each peak timeslot in the game
      /* try
      {
      ...
      // Stores the customer usage related information for each timeslot in the game
      /* try
      {
