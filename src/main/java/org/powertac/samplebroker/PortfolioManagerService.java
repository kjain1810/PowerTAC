 ...
 */
 ...
 /**
  * Included additional methods for data recording
  * and publishing more intelligent tariffs
...
implements PortfolioManager, Initializable, Activatable
{
  ...
  private Map<PowerType, Map<CustomerInfo, CustomerRecord>> customerProfiles;                 // Stores customer_records for each PowerType and each customer
  private Map<TariffSpecification, Map<CustomerInfo, CustomerRecord>> customerSubscriptions;  // Stores customer_records for each tariff and each customer
  private Map<PowerType, List<TariffSpecification>> competingTariffs;            // Stores opponents active tariffs for each PowerType
  private Map<PowerType, List<TariffSpecification>> ownTariffs;                  // Stores own active tariffs for each PowerType
  ...
  private void createInitialTariffs ()
  {
    ...
    for (PowerType pt : customerProfiles.keySet()) {
      ...
      if (pt.isStorage()) {
        ...
        rr.withUpRegulationPayment(-rateValue * 1.45).withDownRegulationPayment(rateValue * 0.5); // magic numbers
        spec.addRate(rr);
      }
      spec.addRate(rate);
      addOwnTariff(spec);
  ...
  private void improveTariffs()
  {
    ...
    if (371 == timeslotIndex) {
      ...
      rr.withUpRegulationPayment(-benchmarkPrice * 0.7 * 1.4).withDownRegulationPayment(benchmarkPrice * 0.7 * 0.54); // magic numbers
      spec.addRate(rr);
      addOwnTariff(spec);
      customerSubscriptions.put(spec, new LinkedHashMap<>());
      tariffRepo.addSpecification(spec);
      ...
      spec.addRate(rate);
      rr = new RegulationRate();
      rr.withUpRegulationPayment(-benchmarkPrice * 0.7 * 1.5).withDownRegulationPayment(benchmarkPrice * 0.7 * 0.46); // magic numbers
      spec.addRate(rr);
      addOwnTariff(spec);
      customerSubscriptions.put(spec, new LinkedHashMap<>());
      tariffRepo.addSpecification(spec);
